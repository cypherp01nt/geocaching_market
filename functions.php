...
<?php		echo "des versteckten Ortes";
	}
}
...
function print_deals_in_escrow($user, $role, $issetDoc, $conn) {
	if ("vendor" == $role) {
		if ($issetDoc) {
			...
			echo '<div class="css_box_long"><h4>Bei folgenden Artikeln ist die Dokumentation des versteckten Ortes <u>nicht</u> an den Kunden verschickt</h4><h5>Der Kunde wird erst das Geld im Escrow an Sie freigeben, wenn er die Ware gefunden hat.</h5></div>';
		}
	} elseif ("client" == $role) {
		if ($issetDoc) {
			$sql_listings = "SELECT id, item, price, quantity, vendor, documentation_link, image 
			...
			echo '<div class="css_box_long"><h4>Bei folgenden Artikeln warte ich auf die Offenbarung des versteckten Ortes durch den Verkeaufer. Ich werde diese unter "Nachrichten" in der Navigationsleiste oben finden.</h4></div>';
			}
	}
	if ($result_listings = mysqli_query($conn, $sql_listings)) {
		if (mysqli_num_rows($result_listings) > 0) {
		// output data of each row
			while($row_listings = mysqli_fetch_assoc($result_listings)) {
				...
				if ($result_escrow_id) {
					...
					$file_6confirmations = fopen("https://testnet.blockchain.info/q/addressbalance/$escrow_address?confirmations=6", "r");
...
?>			<img src="status_is_confirmed_payment.png" alt="status_is_confirmed_payment">
		<div class="css_box_long">
			<h3>order details:</h3>
<?php				echo "Payment arrived the escrow service. The vendor has now been invited to send the documentation of the hidden place to you.<br>".				"The payment gets held by the escrow service until you confirm recivement or 48 hours passed if you dont expand.";
?>			</div>
<?php		}
}
		...
		if ( !$resultListingsPaid = mysqli_query($conn, $sqlListingsPaid) ) {
			...
			while ($rowListingsPaid = mysqli_fetch_assoc($resultListingsPaid)) {
				...
				if ( !isset($rowEscrow['balance']) ) {
					...
					$file6confirmations = fopen("https://testnet.blockchain.info/q/addressbalance/$escrowAddress?confirmations=6", "r");
